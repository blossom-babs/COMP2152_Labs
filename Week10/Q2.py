...
def record_attempt(username, success):
    with sqlite3.connect(DB_NAME) as conn:
        conn.execute("INSERT INTO login_attempts (username, success, attempt_date) VALUES (?, ?, ?)", (username, success, str(datetime.datetime.now())))


def get_failed_attempts(username):
    with sqlite3.connect(DB_NAME) as conn:
        return conn.execute("SELECT * FROM login_attempts WHERE username = ? AND success = 0", (username,)).fetchall()
...
def delete_old_attempts(username):
    with sqlite3.connect(DB_NAME) as conn:
        cursor = conn.execute("DELETE FROM login_attempts WHERE username = ?", (username,))
